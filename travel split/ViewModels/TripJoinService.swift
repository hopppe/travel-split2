...
class TripJoinService {
    ...
    func joinTrip(withInviteCode code: String, completion: ((Bool) -> Void)? = nil) {
        ...
        if Auth.auth().currentUser == nil {
            print("Cannot join trip: User not authenticated")
            tripViewModel.errorMessage = "You need to sign in to join a trip"
            tripViewModel.isLoading = false
            completion?(false)
            return
        }
        
        // User is authenticated, ensure ID consistency
    ...
    func autoJoinTrip(withInviteCode code: String, completion: @escaping (Bool) -> Void) {
        ...
        // Check authentication status first
        if Auth.auth().currentUser == nil {
            print("Cannot auto-join trip: User not authenticated")
            tripViewModel.errorMessage = "Authentication required to join a trip"
            tripViewModel.isLoading = false
            completion(false)
            return
        }
        ...
        // Continue with regular join trip process, just using the provided completion handler
        joinTrip(withInviteCode: code, completion: completion)
    }
} 